...
.PARAMETER NonInteractive
    Never prompt for input. The Git identity (user.name / user.email) is then only
    configured if it is pre-filled in the [General.Git] section of the config file.
    Useful for CI / unattended installations.
...
function Install-GitForWindows {
    ...
    if (-not $wingetAvailable) {
        ...
        if (-not (Test-CommandExists scoop)) {
            Write-Host "  ✗ Neither winget nor Scoop available - cannot install git" -ForegroundColor Red
...
function Initialize-GitConfiguration {
    <#
        Runs directly after the Git installation:
          1. sets the credential helper (default: 'store' -> password is asked only once)
          2. asks for user.name / user.email if they are not configured yet
        Values can be pre-filled in the [General.Git] section of the config file.
    #>
    param([hashtable]$GitConfig = @{})

    if (-not (Test-CommandExists git)) {
        Write-Host "  ⊘ Skipped Git configuration (git not available)" -ForegroundColor Gray
        return
    }

    if ($null -eq $GitConfig) { $GitConfig = @{} }

    # ---------------------------------------------------------------- helper
    $helper = Get-ConfigValue $GitConfig "credential-helper" "store"
    if ($helper -is [bool]) { $helper = if ($helper) { "store" } else { "none" } }
    $helper = "$helper".Trim()

    if ($helper -and $helper -ne "none") {
        $currentHelper = git config --global --get credential.helper 2>$null

        if ($currentHelper -eq $helper) {
            Write-Host "  ✓ Credential helper already set to '$helper'" -ForegroundColor Green
        } else {
            git config --global credential.helper $helper 2>$null
            Write-Host "  ✓ Credential helper set to '$helper'" -ForegroundColor Green
            if ($currentHelper) {
                Write-Host "    (previous value: '$currentHelper')" -ForegroundColor Gray
            }
        }

        if ($helper -eq "store") {
            $credFile = Join-Path $env:USERPROFILE ".git-credentials"
            Write-Host "    Credentials are stored in PLAIN TEXT in: $credFile" -ForegroundColor Yellow
            Write-Host "    You will be asked once on the next push/pull, then never again." -ForegroundColor Gray
            Write-Host "    More secure alternative: credential-helper=manager in the config file." -ForegroundColor Gray
        }
    } else {
        Write-Host "  ⊘ Skipped credential helper (disabled in config)" -ForegroundColor Gray
    }

    # -------------------------------------------------------------- identity
    if (-not (Get-ConfigValue $GitConfig "configure-identity" $true)) {
        Write-Host "  ⊘ Skipped Git identity (disabled in config)" -ForegroundColor Gray
        return
    }

    $currentName  = git config --global --get user.name 2>$null
    $currentEmail = git config --global --get user.email 2>$null

    $configName  = "$(Get-ConfigValue $GitConfig 'user-name' '')".Trim()
    $configEmail = "$(Get-ConfigValue $GitConfig 'user-email' '')".Trim()

    # Config file wins over an existing value only if it is explicitly set
    $targetName  = if ($configName)  { $configName }  else { $currentName }
    $targetEmail = if ($configEmail) { $configEmail } else { $currentEmail }

    if (-not $targetName -or -not $targetEmail) {
        if ($NonInteractive -or -not [Environment]::UserInteractive) {
            Write-Host "  ⚠️  Git identity incomplete and running non-interactively" -ForegroundColor Yellow
            Write-Host "    Set user-name / user-email in [General.Git] or run:" -ForegroundColor Gray
            Write-Host '      git config --global user.name "Your Name"' -ForegroundColor Gray
            Write-Host '      git config --global user.email "your@email.com"' -ForegroundColor Gray
        } else {
            Write-Host "`n  Git identity is used as the author of every commit." -ForegroundColor Cyan
            while (-not $targetName) {
                $targetName = (Read-Host "  Your name (git user.name)").Trim()
            }
            while (-not $targetEmail) {
                $targetEmail = (Read-Host "  Your e-mail (git user.email)").Trim()
            }
        }
    }

    if ($targetName -and $targetName -ne $currentName) {
        git config --global user.name $targetName 2>$null
    }
    if ($targetEmail -and $targetEmail -ne $currentEmail) {
        git config --global user.email $targetEmail 2>$null
    }

    if ($targetName -and $targetEmail) {
        Write-Host "  ✓ Git identity: $targetName <$targetEmail>" -ForegroundColor Green
    }
}
...
function Get-ConfigValue {
    param(
        [hashtable]$Config,
        [string]$Key,
        # Untyped on purpose: values can be booleans (tool switches) or strings
        # (e.g. the Git identity in [General.Git])
        $DefaultValue = $false
    )
...
if ($ToolsUserRights) {
    ...
    # INSTALL GIT FIRST
    # ========================================================================
    # Git is the foundation of the whole environment, so it is installed before
    # Scoop and every other tool. Directly afterwards the credential helper and
    # the commit identity (user.name / user.email) are configured.

    if ($ForceInstall.Count -eq 0) {
        Write-Section "Git (Bootstrap)"

        $coreToolsBootstrap = $config.UserLevel.CoreTools
        Install-GitForWindows -ShouldInstall (Get-ConfigValue $coreToolsBootstrap "git" $true)

        # Refresh PATH so a freshly installed git is found in this session
        $env:Path = [System.Environment]::GetEnvironmentVariable("Path", "Machine") + ";" +
                    [System.Environment]::GetEnvironmentVariable("Path", "User")

        Initialize-GitConfiguration -GitConfig $config.General.Git
    }
    ...
    if ($coreTools) {
        # Git was already installed and configured in the "Git (Bootstrap)" section above
...
4. Git is already configured (identity + credential helper).
