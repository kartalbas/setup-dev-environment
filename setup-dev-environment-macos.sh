    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ "$line" =~ ^\[(.+)\]$ ]]; then
            ...
            if [[ "$full_section" == "General" || "$full_section" == "SystemRequirements" ]]; then
                ...
                subsection=""
            elif [[ "$full_section" =~ ^General\.(.+)$ ]]; then
                section="General"
...
get_config_value() {
    local key="$1"
    # Use ${2-false} (not ${2:-false}) so an explicit empty default stays empty
...
configure_git() {
    ...
    if ! command_exists git; then
        print_gray "Skipped Git configuration (git not available)"
        return
    fi

    # ------------------------------------------------------------- helper
    local helper
    helper="$(get_config_value 'General.Git.credential-helper' 'store')"

    if [ -n "$helper" ] && [ "$helper" != "none" ] && [ "$helper" != "false" ]; then
        local current_helper
        current_helper="$(git config --global --get credential.helper 2>/dev/null || true)"

        if [ "$current_helper" == "$helper" ]; then
            print_success "Credential helper already set to '$helper'"
        else
            git config --global credential.helper "$helper"
            print_success "Credential helper set to '$helper'"
            if [ -n "$current_helper" ]; then
                print_gray "(previous value: '$current_helper')"
            fi
        fi

        if [ "$helper" == "store" ]; then
            print_warning "Credentials are stored in PLAIN TEXT in: $HOME/.git-credentials"
            print_gray "You will be asked once on the next push/pull, then never again."
        fi
    else
        print_gray "Skipped credential helper (disabled in config)"
    fi

    # ----------------------------------------------------------- identity
    if [ "$(get_config_value 'General.Git.configure-identity' 'true')" != "true" ]; then
        print_gray "Skipped Git identity (disabled in config)"
        return
    fi
    ...
    target_name="$(get_config_value 'General.Git.user-name' '')"
    target_email="$(get_config_value 'General.Git.user-email' '')"
    if [ -z "$target_name" ]; then target_name="$current_name"; fi
    if [ -z "$target_email" ]; then target_email="$current_email"; fi

    if [ -z "$target_name" ] || [ -z "$target_email" ]; then
        if [ "$NON_INTERACTIVE" == true ] || [ ! -t 0 ]; then
            print_warning "Git identity incomplete and running non-interactively"
            print_gray "Set user-name / user-email in [General.Git] or run:"
            print_gray "  git config --global user.name \"Your Name\""
            print_gray "  git config --global user.email \"your@email.com\""
        else
            echo ""
            print_info "Git identity is used as the author of every commit."
            while [ -z "$target_name" ]; do
                read -r -p "  Your name (git user.name): " target_name
            done
            while [ -z "$target_email" ]; do
                read -r -p "  Your e-mail (git user.email): " target_email
            done
        fi
    fi

    if [ -n "$target_name" ] && [ "$target_name" != "$current_name" ]; then
        git config --global user.name "$target_name"
    fi
    if [ -n "$target_email" ] && [ "$target_email" != "$current_email" ]; then
        git config --global user.email "$target_email"
    fi

    if [ -n "$target_name" ] && [ -n "$target_email" ]; then
        print_success "Git identity: $target_name <$target_email>"
    fi
}
...
PARAMETERS:
  ...
  --non-interactive
      Never prompt for input (CI / unattended installs).
      The Git identity is then only set if it is pre-filled in [General.Git].
...
if [ "$TOOLS_USER" == true ]; then
    ...
    # every other tool. Directly afterwards the credential helper and the commit
    # identity (user.name / user.email) are configured.

    print_section "🔑 Git (Bootstrap)"

    install_brew_package "git" "Git" "$(get_config_value 'UserLevel.CoreTools.git' 'true')"
    configure_git
fi
...
EOF
    ...
    echo "   go version"
    echo ""
    echo "3. Git is already configured (identity + credential helper):"
