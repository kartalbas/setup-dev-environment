...
[General.Git]
credential-helper=store         # store password/token once (or: manager / none)
configure-identity=true         # prompt for user.name / user.email if unset
user-name=                      # pre-fill to run unattended (else prompted)
user-email=
...
Git is installed **first**, before every other tool. Right after installation the
scripts configure it from the `[General.Git]` section:

1. **Credential helper** — `store` (default) saves your password / personal access
   token so you are asked only **once** on the next `push`/`pull`. The credentials
   are written in **plain text** to `~/.git-credentials` (`%USERPROFILE%\.git-credentials`
   on Windows). Use `manager` (Windows) for encrypted storage, or `none` to leave it
   untouched.
2. **Identity** — if `user.name` / `user.email` are not set yet, you are prompted for
   them. Pre-fill `user-name` / `user-email` in the config to run unattended.

For fully automated / CI runs, add the non-interactive flag so the scripts never
prompt (the identity is then only set if pre-filled in the config):

```powershell
# Windows
.\setup-dev-environment-windows.ps1 -ToolsUserRights -NonInteractive
```

```bash
# Ubuntu / macOS
./setup-dev-environment-ubuntu.sh --user --non-interactive
```
